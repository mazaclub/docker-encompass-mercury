...
COINDIR=${COINDIR:-/home/${USER}/.${COIN}-v2}
RPCPORT=${RPCPORT:-9347}
...
if [ "${TXINDEX}" = "1" ] ; then
   echo "-txindex is set    good to go"
     ...
     touch /etc/service/startcoind/down
     echo "Now you can start clamd manually with -reindex and then add:"
     echo "txindex=1"
     echo "to your ${COINDIR}/startcoin.conf"
...
exec /app/run_encompass-mercury_server --coin ${COIN_SYM}
